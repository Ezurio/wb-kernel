				 ...
				 struct cfg80211_connect_params *sme)
{
	...
	if (sme->crypto.wpa_versions & NL80211_WPA_VERSION_1) {
		...
		else if (drvr->bus_if->fwvid == BRCMF_FWVENDOR_CYW &&
...
BRCMF_EXPORT_SYMBOL_GPL(brcmf_update_pmksa);
...
static int
brcmf_cfg80211_update_owe_info(struct wiphy *wiphy, struct net_device *dev,
...
static s32
brcmf_mchan_config(struct brcmf_cfg80211_info *cfg)
{
	struct brcmf_pub *drvr = cfg->pub;
	struct brcmf_if *ifp;
	int ifidx = 0;
